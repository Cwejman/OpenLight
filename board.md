...
2. **Author's ratification read of the redistributed specs** — substrate.md, engine.md (the new Program/Process, Lifecycle, Dispatch, Boundaries, Expressions sections), programs.md, bootstrap.md whole; db/sdk/agent/session/host/pilot at the changed passages. The steward wrote; the author ratifies. Particular attention: the frame-as-ownership-subtree reading (engine.md, marked open) and the `demand` absent-vs-empty rule (engine.md §Boundaries).
