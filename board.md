...
- **THE AUTHORITY for the coming rewrite: [`spec/research/settled.md`](spec/research/settled.md)** — the whole settlement, current, one file. threads-dialog.md is the path with superseded layers; do not read it as current.
...
- **Build track paused at the dispatch frontier** (author ruling): a program launch is arguments/grants/forms/landing, and programs.md holds those as round-two bullets, not a drawn contract. Planning runs in author dialog until the contract exists. **Pause lifts when the redistribution lands** — settled.md §2 draws the draft/form/run/launch contract the pause waited for.
...
2. **Spec folds — LANDED** (co-authored session, 2026-08-02): programs.md holds the four contracts (`reader` §3.5 — readings, collation, bindings; `process-view` §3.7 — three regions, result = frame ∩ declared archetype, editability = unconsumed, stale-display; `form` §3.4; `prose` §3.5) + the immutable-argument rule (§1) + the draft state (§2) + §3.6 rewritten to the dissolution + the **result-type naming pass** (§4); agent.md: context as expression + resolution, thread derived, draft-summons-form; session.md rewritten whole to the thread model; substrate.md: §Grain + Views→reading/template; engine.md: `explode` open. Vocabulary since resettled in the author read-pass (Namings above). **Author's fresh read of the rewritten session.md and programs.md §3.4–3.7 still owed** — the co-author wrote, the author ratifies.
