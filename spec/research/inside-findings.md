...
- **2026-04-05 onward** — interface.md added, shell grounding shifted toward agent-case, spec/pilot.md specified (TypeScript/Bun, SvelteKit, VM containment). By 2026-04-10, `browser/` and `openlight/` archived, pilot directory appeared.
...
Current root files have pushed this entirely into spec/pilot.md's "what's open" and stripped the visionary framing.
...
5. **The lower-level LLM cycle vision pushed to "what's open".** Originally central: integrating completion at the generation level, dissolving the boundary between model and environment. Currently: a footnote in spec/pilot.md's open questions.
...
- `spec/pilot.md` open questions section — read as a list of value-shaped questions that want value-shaped answers.
...
Each major refinement pass (03-08 → 03-15 breakthrough → 04-04 restructure → 04-05 interface/shell refinement → spec/pilot.md) improved precision and reduced bloat. That work was not wasted. But each pass also stripped out one more layer of felt language, metaphor, humility, philosophical lineage, and visionary framing — the things that are hard to justify per-sentence but carry the whole thing together. The current root files are the skeleton. `inside.md` is an attempt to restore the part of the body that orients the skeleton. These findings are the muscle memory of how the thing has been held over the past year.
