...
The engine exposes only Rust functions; it ships no TS client. Programs reach those functions through the SDK, which serializes each call into the protocol JSON and selects the transport. The SDK ships from the engine crate ([`engine/sdk/`](../engine/sdk/)) because it is the engine's protocol expressed as TypeScript; its surface and the two transports are specified in [`sdk.md`](sdk.md).
...
- **`explode` — virtual chunks from body keys.** A pure transform that projects a JSON body's keys as virtual chunks, each with its own instance placement — the same family as `db/commits` and `engine/mount`, synthesized at query time rather than stored. The principle under it: **pipe output is substrate-shaped** — chunks-and-placements — so scope algebra composes over transform results, not just stored scopes. Materializing is committing the same output (one program, two sinks); promotion when a query proves hot, never upfront, the body never ceasing to be truth. Grain ruling in substrate.md §Grain decides what is *stored* as a dimension; explode manifests the rest on demand. Shape unbuilt; lands with the pipe vocabulary (programs.md §3.5).
