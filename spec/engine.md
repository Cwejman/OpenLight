...
- **Process creation.** Dispatching a program writes a `process` chunk in one atomic `db.commit()` — its body and connections are defined in *Program and Process*. From dispatch on the process chunk is engine-domain: a running program cannot rewrite its own record.
- **Boundary enforcement.** Every scope read, every write, every nested program run is checked. A run's reach is constructed at dispatch — its own frame, its granted roots, its program's demand — and walked through ownership. Reads outside it return `BOUNDARY_VIOLATION`; writes outside it are rejected.
- **Program lifecycle.** The engine spawns the program's executable, tracks its status through `running → done | failed` (a `draft` precedes dispatch and is data, not engine-domain), updates the process chunk as state changes, kills on timeout or cancel. The program itself does not set its status — it simply exits.
...
`engine/program` is the archetype every runnable thing is `instance` on. Its instance spec types the program body:
...
`engine/process` is the archetype every run is `instance` on. Its instance spec types the process body — every key statically typed:

```
{ argument: ref          — the argument chunk; frozen wholesale at dispatch
...
**Validation is one placement check.** At dispatch: is the argument chunk an instance of what the program's `argument` names. At completion: is the result chunk an instance of what the program's `result` names. Nothing else. The former accepts machinery — union composition, per-list ambiguity judgment, the trace-nesting exemption, federated accepts pre-validation — retires with the spec language; a child process placed on its parent is just a placement, nothing rejects it. (The built engine still carries that machinery; retirement is tracked on the board, spec leads.)
...
process P
  ...
  instance on: engine/process, <program>, <caller-supplied scopes, e.g. the session>
...
|---|---|
| `scope` | Read the intersection of scopes. Filtered by the effective read boundary. Membership across the three stored kinds plus the `linked` answer, per substrate.md (*Scope*). FTS via `ScopeOpts.match_`; an **empty scope list with `match_`** is a whole-field FTS query, boundary-filtered and federated like any read. Negation via `exclude`. Pagination and body-less projection per substrate.md. |
...
| `read_batch` | Multiple tagged `scope`/`get` sub-queries resolved together at **one commit snapshot**, each authorized under its own identity (see *Multiplexed transports*). One request, coherent results — the resolution primitive behind slot-and-hook views (programs.md). |
| `commit` | Write a Declaration. Rejected if any chunk or placement touches a scope outside the write boundary; ref keys validate per substrate.md (federated through the mount registry). `dry_run: true` runs full validation without writing — the live-form affordance. |
| `run` | Dispatch a program. Returns the process id immediately. Takes a program + argument chunk, or a `draft` process id to consume. `mode: 'child' | 'launch'` per *Lifecycle*. |
...
{"id":9,"op":"exit"}
{"id":10,"op":"subscribe","scopes":["my-session"]}
...
| `scope` | `ScopeResult` |
| `get` | `ChunkItem \| null` |
| `read_batch` | `{ head: CommitId, results: Record<tag, ScopeResult \| ChunkItem \| null \| EngineError> }` |
...
The wire carries the tagged value encoding for typed bodies (`$ref`, `$loc`, `$set`, `$time`, `$md`) — translation is the SDK's job ([`sdk.md`](sdk.md)); the engine validates tags against instance specs at commit.
...
| `VALIDATION_ERROR` | Declaration fails spec validation — instance-spec key check, ref-target check, or the dispatch/completion placement check |
...
| Event | Shape | Meaning |
|---|---|---|
| `scope_changed` | `{ event: "scope_changed", subscriptionId, commit }` | A commit touched a scope this subscription registered on. The SDK re-fetches via `scope`. |
...
| `subscription_invalid` | `{ event: "subscription_invalid", subscriptionId, reason }` | A subscribed scope became unreachable from the process's read boundary. The engine has unsubscribed; the SDK treats the subscription as dead. |
...
`run` dispatches and returns the process id immediately. The spawned program runs on its own. `await` waits on a set of process ids until they reach terminal state — it suspends the calling task, not the engine. There is no structural difference between spawning an agent and calling a tool — a filesystem read returns in milliseconds, a sub-agent might run for minutes; the protocol handles both identically.
...
Parallel calls are several runs awaited together; fire-and-forget is a run awaited later. Every process chunk exists in the substrate immediately — any program within its boundary can scope into a running process and watch.
...
pub struct RunArgs {
    ...
    pub placements: Vec<ChunkId>,       // additional instance scopes for the new
...
pub enum RunTarget {
    Dispatch { program: ChunkId, argument: ChunkId },
...
**Federated reads and boundary.** `scope` and boundary walks iterate the mount registry. Reads union and dedupe across mounts; ownership walks stop at mount edges (ownership never crosses mounts — substrate.md); `linked` answers union per-db link tables. Programs see one field. Unresolved roots federate by **intersection**: a root lands in `ScopeResult.unresolved` only when no mount resolves it.
...
**Boot-time validation.** Before entering the event loop the host asks the engine to validate the active project: every placement's `scope_id` must resolve in some mount. The engine returns unresolved references; the host surfaces them and refuses to run half-loaded.
...
**Sync vs async.** The substrate is sync (SQLite is sync), so `scope`, `commit`, `run`, `subscribe`, `unsubscribe`, `cancel`, mount ops return without awaiting. `await_processes` and `shutdown` are async. Outgoing event delivery to webviews rides the `HostCmd` channel returned at `Engine::open`.

---

## Dispatch — what the engine writes
...
A run's reach is **constructed, not filtered** — assembled at dispatch from three sources, recorded as the process body's `read` and `write` keys, immutable for the run:

1. **The frame.** Every process reaches its own ownership subtree, both ways, always. Implicit; never listed. The argument chunk rides along (implicitly granted at dispatch).
2. **Grants.** Roots derived from the argument's `grants: read|write`-marked ref keys as filled, plus whatever the dispatching person or program adds. For nested runs, the child's grants are intersected with the parent's effective reach at spawn — reach narrows through the call stack, never widens.
3. **Demand.** The program body's argument-independent residue. **Absent means open** — the program defers reach entirely to the run (the agent). **Present means exact** — the program's argument-independent reach is precisely this, and a run may narrow it, never widen it; `demand: { read: [], write: [] }` is the frame-only program (`model` — nothing beyond its frame, enforced, not promised). *Open: `demand`'s final shape (settled.md §7).*
...
**Protected records.** From dispatch on, the engine rejects any program write that modifies the process chunk itself (status, result, grants — engine domain) or a consumed argument.
...
call:        { program: diff, args: { old: a, new: b } }
             — named args always; param names from the argument archetype's
               instance spec; values are literal | ref | name
...
**Plan-form vs run-form.** Nodes hold their args inline as data — the plan. Running a node materializes the argument chunk (`instance` on the program's argument archetype) and dispatches — the same call frame as every run; expressions add no second execution path. Type-checking a plan is reading the argument archetypes' instance specs, before anything runs.
...
How a `subscribe` op becomes a `scope_changed` event in the calling program.
...
2. **engine.** On `mount_project` for a `ReadWrite` mount, the engine subscribes to that mount's `db.subscribe_scope(&[db/commits], ..)`. A background task drains the receiver, filters by the mount's branch, and runs the dispatcher.
...
3. **dispatcher.** For each incoming `Commit`, the engine computes the *touched scope set* — the union of:
   ...
   - Both sides of `commit.placements_modified` — scopes that gained or lost a placement, and chunks whose own placements changed.
   - `commit.links_modified` — chunks that gained or lost links *to* them (the link delta, computed in the write transaction).
   - For each chunk in `chunks_modified`, the scopes it is currently placed on (all three stored kinds) — so a subscriber on a parent sees a member's body change. One bulk lookup per commit.

   The dispatcher fires `scope_changed` on every subscription whose scopes intersect the touched set.
...
5. **SDK.** Distinguishes by message shape, routes to the subscription's callback; `useScope` re-fetches and re-renders.

### Subscription lifecycle

- `subscribe(ctx, scopes)` — boundary-checked against the process's read reach. On pass: registered, id returned. On fail: `BOUNDARY_VIOLATION`, delivered by the SDK as the dead-subscription path (sdk.md).
...
Grants are immutable, but reachability through them is dynamic — an ownership change or removal elsewhere can sever the path from a run's roots to a subscribed scope. On every commit whose placement delta could affect it, the engine recomputes reachability for possibly-affected subscriptions; severed ones are removed and `subscription_invalid` fires with a short reason. After that, no further events for the subscription. The dumb recompute-per-affected-process is v0.1; dependency tracking is a deferred optimization.
...
**Coalescing is required, not deferred.** The streaming convention makes commit bursts normal, so multiple commits touching a subscription within a short window fire one `scope_changed` (carrying the latest commit). Invisible to correct clients — the contract is re-fetch on event.
...
`ProcessStatus` is one enum used in-memory and at the substrate body field (as the status value-chunk ref). Slots exist only for dispatched processes — drafts are data, never slotted. The map is `HashMap<ProcessId, ProcessSlot>` under a Mutex; slots are created on dispatch and removed on terminal transition.

### `run` (dispatch)

The slot is inserted *before* the substrate write so `cancel` and `timeout` can always land on a known process id.

1. **Placement check + compose the declaration** (see *Dispatch*).
...
The dispatch commit writes `status: running`; the commit-to-spawn gap is engine-internal, never a field state. A cancel landing in the gap is caught at step 4 or by the watcher tasks; the substrate always carries a complete record and `await_processes` resolves to the terminal state.
...
Nothing discourse-shaped is written anywhere — the tool trace *is* the frame; providers wanting message history get it reconstructed from frames as serializer policy ([`agent.md`](agent.md)). Substrate operations (`scope`, `commit`, `subscribe`) from the agent are not tool calls — they go directly through the protocol and create no processes.

---

## Traceability

Every commit carries a `process_id` — the run that caused it, or null for host-level commits. Commits stay in their own table; the read layer projects them under the virtual scope `db/commits`:

- `scope([db/commits])` — all commits
- `scope([db/commits, processId])` — commits from this run
- `scope([db/commits, chunkId])` — commits that modified this chunk

Chunk → commit → process → program: any change walks back to the program that caused it and the person who ran it. Virtual scopes accept the parameter shapes listed in [`db.md`](db.md#virtual-chunks-branches-and-commits); unrecognized shapes return empty, never error.
...
|---|---|
| Boundary violation (scope, subscribe, commit) | `BOUNDARY_VIOLATION` response; process continues |
...
An explicit `BOUNDARY_VIOLATION` beats a silently empty read. The engine returns the error when a queried scope isn't reachable, so empty results mean genuinely empty scopes, not withheld ones.
...
engine/
  src/
    ...
    bootstrap.rs        — reconcile_zombies(&Db): one scope query, one commit.
    ...
    expressions.rs      — the expression shapes; parse (recursive descent);
                          plan type-check against instance specs; run-form
                          materialization
    ops/                — public surface; one module per Engine method
      scope.rs get.rs commit.rs run.rs cancel.rs subscribe.rs await_processes.rs
...
**State authority follows lifecycle.** A dispatched process has two homes — its slot (live runtime) and its substrate chunk (durable). The slot is authoritative while active; the substrate once the slot is gone. Authority transfers in one ordered step at terminal: cleanup writes the terminal status, then drops the slot. One truth at any moment; the seam is the cleanup commit.

**Reactivity owns event emission.** The reactivity task is the engine's only consumer of the db change feed and the only emitter of `scope_changed` / `lagged` / `subscription_invalid`. Cleanup paths trigger reactivity by writing terminal commits; they never emit events directly.
