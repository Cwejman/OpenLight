...
The table is checked top-to-bottom and the first match wins — its rows are not disjoint (an ordered scope whose instances share a schema is Sequence, not Table), and **Cards is the catch-all, checked last**, whatever its display position above. Roots the read resolves nowhere (`ScopeResult.unresolved`) render as an explicit unresolved state — a dead reference, distinct from empty: an empty scope invites, a dead root does not. Where a chunk has no name, display falls back to its truncated monospace id. Only Document mode distinguishes `instance` from `relates` placements — the v0.1 default; *open for revisit* as other modes earn the split. Errors on referenced chunks the view reaches — `NOT_FOUND`, `BOUNDARY_VIOLATION` — render inline, quietly, at the point of reference (`edit`'s `VALIDATION_ERROR`-as-form-validation precedent).
...
### 3.6 The conversation — dissolved into the field

**Ruling: there is no conversation program.** Walk what `converse` supposedly did through everything since settled, and nothing remains: the transcript is `read` in sequence mode; the input is the dispatcher citizen; gates and controls are citizens; per-message rendering is entity renderers. "Conversation" names two things only — an **archetype**, `{ordered: true}` with content deliberately wildcard, typing the *container* so surfaces can match (never the content, and never an agent-session kind); and the **derived view** that assembles when you open one: reader ground, dispatcher peer, entity renderers, per §3.5. The flagship experience of the system costs zero new surface programs. (Where a transcript wants behavior beyond `read`'s sequence modes — tail-follow, author grouping — the escape hatch is the peer model itself: a specialized ground citizen swaps in.)

**What accumulates on a conversation** (authority for agent sessions: [`session.md`](session.md) — turns only, prompt on the invocation, answer on the frame; messages belong to human conversations):
- **Messages** — from any participant; prompts and answers are both messages, `body.from` carries the face.
- **Invocation entities** — the turn's *process chunk itself*, dual-placed onto the conversation with seq at dispatch: the work appears in the timeline while its mechanics (tool runs, context items, verbatim requests) stay solely on frames — one source, reached by drilling. Rendered by the invocation renderer: folded, a live one-line form; expanded, the nested runs recursively.
- **Summary chunks** — placed on the entities they abstract: the members now share a common chunk, that chunk **is** the group, folding renders it in their place, and it has its own first-class renderer. (This is the substrate's oldest pattern — the chunk at the intersection is the relationship — and it replaced a range-reference mechanism we almost invented.)

Everything addressable still renders inline at its natural representation — a scope, a chunk, a single field on an entity — small in line, larger as a fold that **expands in place**; a tile is an option, never the only path. Third-party entity types join by declaring type and renderer, touching nothing.

**The dispatcher** — one citizen: compose, launch, and context selection. Matched on `conversation` by acceptance-as-placement — **and only on `conversation`**: the type is the agent's answer-home requirement made mechanical. An email thread (`email/thread`, an integration's type) never grows a dispatcher; its own reply composer matches its own types and answers in its own medium; *talk about this* bridges it into a conversation, where the thread rides as context and the agent answers as a message (see [`agent.md`](agent.md), *the answer-home is typed*). Its input row writes messages and dispatches turns; its expanded form composes the agent's **ordered context list** — guidelines, the conversation, the prompt, this turn's additions. Its standing **selection is a field entity** (`relates` on the conversation; per-item projection full / summary / name), and the reader highlights included entities from it as you scroll — **citizens coordinate through the field, never through props.** Boundary chips and toolset stay visible before anything runs. Launches are detached (§2); a branch selector (R1) flips the next turn onto a work branch, `merge` (§3.8) brings it home.

**Interaction in the timeline.** Every entity carries its verbs (§3.5): *add to context* (a dispatcher verb), *talk about this* (opens a conversation related on the entity), *review changes* on an invocation, approve/deny on gates surfaced by the invocation renderer — with the rule that **a folded invocation's live obligations penetrate the fold**. Streaming is partial commits (R6); pause/resume are `control` chunks ([`agent.md`](agent.md)).

**Starting and relating.** A conversation starts empty, or pre-related from *talk about this* — opened already carrying the entity as a relation, the way you'd open a file and start talking about it. Relations are placements, so a conversation about a plan section sits *on* that section for anyone who later reads it.

*Open:* gate placement — frame-only, or also placed onto the conversation while its invocation runs, so it surfaces in the timeline (the author leans possible; undecided); whether **participants** as relations are the inviting mechanism; selection expression (ids in the selection chunk's body vs placements — lean body ids). All settle with the conversation tile.
...
| `dispatcher` | webview | compose + dispatch + context selection; the conversation's write-side citizen (§3.6) — `converse` dissolved into `read` + citizens |
