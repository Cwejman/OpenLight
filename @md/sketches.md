...
**Grounding.** The taxonomy study settled that the markdown holds one direction and a frontend derives the rest, the reverse trail from the links and the history from the commits ([`evidence-series.md`](spec/research/knowledge/study/08-win-driven/08-01-ontology/evidence-series.md), *What the other side shows*). The ontology now carries comment, commit, space and trail as words ([`ontology.md`](spec/research/knowledge/study/08-win-driven/08-01-ontology/ontology.md)), so the sketch stands on named ground. A comment is the same kind of thing: a connection to a holon that the holon does not carry, shown by whatever reads the shape. Prior art is every publishing surface with a comment thread beneath the post, and the project's own convention that records are events: a comment that changes the holon is folded and retires, and git keeps it.
...
**The smallest build** (steward, 2026-09-04). A read-only holon list: parse the markdown tree into a flat list of holons with address, surface, depth, parent, children and links out; compute the inverse of every link; show each holon with what it is within, what it stands on and what stands on it; show each commit and its changes browsed by the same relations; show comments as holons standing on what they comment, walkable by knower or by space. Centrality becomes measurable as the count of what stands on a holon, and the parked expand-and-collapse open unblocks since the page is ours. The existing `.wiki/` pandoc build knows files and folders only; the smallest step is a pass that emits the holon list as one JSON file.
