...
Beyond the opens marked in place above: grid or composed lists · the spacing/highlight law (`design-language.md`) · two runs of one program with identical arguments told apart (the sidebar — [`desktop.md`](desktop.md)) · state-dependent menu enablement (entries static in v0.1) · the accepts-side payload archetypes' names (bootstrap) · N-source contexts in the thread face (author-flagged).
