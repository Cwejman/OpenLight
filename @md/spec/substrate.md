...
- `selection` — `set<loc | ref | expr>`, with a purity clause: expressions within one must derive pure ([`engine.md`](engine.md)). A selection is what you offer to be viewed or consumed: chunks, places, and derivations of places. **A chunk and the place at it are different offers** — `X` is the chunk alone; `[X]` is the place at X, which carries the chunk *and* what is placed on it. Both are legitimate things to hand someone, so a selection admits both; without `ref` you would need an expression subtracting a place's members from itself to say "here is this one thing." **At most one `selection` entry in a program's `accepts`** — two would compete for the same offered elements ([`engine.md`](engine.md)). This is a rule about argument matching, not about contracts at large: a body may hold several selection-typed keys, as the program contract's `read`, `write` and `run` do.
