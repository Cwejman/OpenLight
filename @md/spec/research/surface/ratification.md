...
- **4** narrowed: no block grammar — `list` and `split` as base components; grid and the spacing/highlight law open (the law's full reasoning preserved in [`design-language.md`](design-language.md)); derived arrangements and the WYSIWYG inversion specced (drag deferred after the first pilot).
