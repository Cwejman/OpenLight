...
**Amended 2026-08-20 — the feedback fold** (five author notes on the read; `feedback.md`, committed then removed — git keeps it): sources engine-served **[R]** · the mount context became the **call context** — the engine's own structural contract, never naming view · the home is a **standard store directory** (`field.ol` was the deviation) · the **simple tiler** — tabs and sidebar out of the pilot shell · **the session's standing held open** — the talk-over owed before the rewrite touches its sections.
...
- **The call context [R — reshaped in the feedback fold: the engine never names view].** The engine already judges a process by its call — declaration (the program, whose ceiling applies), argument, grant, parent. The fold makes that shape the engine's own **structural contract**: a conforming chunk names a *declaration* carrying ceilings, an *argument* (a selection), its own *grant* (`read`/`write`/`run` additions), and has a derivable *parent* conforming chunk. Effective reach at a link = (the argument, read-granted as any argument is ∪ the declaration's ceiling ∪ the grant) ∩ the parent's; the chain caps at the **machine context** — `Context::process_id = None`: full reach over what is attached, every created chunk naming its owner. `Context` gains **`{ anchor }`** — a ref to any conforming chunk; batch reads carry the anchor per entry; the engine derives the chain from field data — cacheable — and judges `read` / `commit` / starts exactly as for a process; rendering under a mount reads under its anchor's reach. `engine/process` conforms by construction; **`view/mount` conforms from the view side** — the engine never references a view archetype (the dependency law holds at this seam too: view depends on engine, never the reverse). **Intents are dissolved [R]**: what interface code emits is an ordinary `commit` or start. How the parent link is derived (containment backrefs vs a stored key) [O — encoding].
- **Starts from a mount are `launch` [P].** ("mount" here and below is view's — *view/mount* — never the attached store.) A mount has no frame; a process it starts is owned by the session, capped by the mount's reach (the `run` wall says *which* programs; `launch` is the mode), watched by subscription. Other ownership mechanics when users and permissions arrive — out of scope. (The session's standing is held open — *the desktop module*; if it dissolves, the engine wording generalizes to whatever owner the entry names.)
...
Opens: an act journal as the escape if a truly atomic cross-store case appears (never spanning commits) · dynamic-attach encoding · ops beyond read/write/run under `{ anchor }` (`cancel`, `subscribe`) · the call chain's parent derivation · the consent chord's principal (lean: the mount of the draft face — *component/process*).
...
**What you run is the engine, and a chassis.** Two commands, one directory — **and the home is a store directory like any other [R — the feedback fold; `field.ol` was the deviation]**, recognized as every store is: `.ol/` inside, db and toml within (`project.toml`'s successor, one shape everywhere):
...
- **the shell — the simple tiler [P — the feedback fold's cut; tabs and sidebar out of the pilot shell].** `desktop/shell : view/component { accepts: [ ref(session) ] }` realized **as a template** placing `$session.root` — a tree of `split` and content mounts; the arrangement chunks shrink to `session { root?: ref(view/mount) }` (`tab` dies with the tab strip; `host/tile` and `host/overlay` were already gone). The tiler is verbs and one gesture, no mechanism of its own: **split** — a command verb from the match (the module declares `actions` over `ref(view/mount)` payloads; reached by the secondary gesture), wrapping the target mount in a new `split { direction, [target, new] }`, the new pane filled by the defaults table's last resort until content is chosen; **close** — the ruled body edit, removing the mount from its parent's list, a single-child `split` collapsing into its child (the verb's own normalization); **resize** — the divider is the split implementation's own pointer interaction (the ordinary gesture belongs to the component in use), committing `ratio`; **move/swap** — post-pilot, riding the drag layer, and WYSIWYG rearrangement with it; the tiler needs nothing from that layer to be usable. With tabs gone the shell is nearly transparent — its remaining duty, resolving *which* session, rides the session question [O, below];
...
- **the session's standing [O — the owed talk-over].** The brief holds session in four roles — `launch` ownership, input-record ownership, the shell's argument, the arrangement root. Whether it survives as desktop data (the engine generalized to a configured owner — *engine*, `launch`) or dissolves entirely is the next dialog, before the rewrite touches these sections;
...
db: id-debt schedule · engine: act journal, dynamic-attach encoding, ops beyond read/write/run under `{ anchor }`, the call chain's parent derivation, the consent chord's principal · view: the ref refinement for closed slots (new type grammar), expressions-in-fields encoding (slots, templates, offers), nearest-vs-root ordering for `draw`, the duplicate-realization governance rule's exact statement, when code-creating mounts arrive · web-dom: record and connection encodings, ephemera home, web input floor, the slot-drop interface · base: grid or lists, the spacing/highlight law · desktop: **the session's standing (the owed talk-over)**, sidebar and mounts (3.3), the projects component's shape · reader: 5.3 / 5.4 / 5.5, differencing · table: 5.6 · command: scope beyond commands, a mount-history browse (3.4) · chassis and home: flavors, reservation encodings, the attach-time consent chip, the seed's contents · **to prototype before folding: `ctx`, the adapters and the typed-argument ergonomics — the authoring experience.**
