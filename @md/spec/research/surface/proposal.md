...
2026-08-19. The brief the spec rewrite executes from — the surface arc's result, ratified in dialog over 08-15…19 (the ratification record was removed 2026-08-20 after its live rows folded in — git keeps it; walked grounding for acts and walls: [`act-scenes.md`](act-scenes.md)). **Status: ratified brief; nothing is folded into the spec tree yet — the next sitting rewrites the tree from this file.** The two prior generations (`research/arc/surface-proposal.md`, the first `research/surface/proposal.md`) are deleted; git history keeps them.

**Amended 2026-08-20 — the feedback fold, corrected same-day on the author's review** (five author notes on the read; `feedback.md`, committed then removed — git keeps it): sources engine-served **[R]** · the mount context became the **call context** — the engine never names view [R]; the contract's shape and the `anchor` name are steward-drafted [P] · the home is a **standard store directory** (`field.ol` was the deviation; established naming kept — `.ol/project.toml`) · the **simple tiler** — tabs and sidebar out of the pilot shell [R]; the verb set steward-drafted [P] · **the session ruled**: minor state of the pilot desktop module; the engine's `launch` owner is configured · **daemons ruled** (ratification 3.2): impure programs launchable like any, purity never the gate; nothing auto-starts — user-started services, the honest face when absent · the knowledge-structure question (7.1) recorded as an open point in `conventions.md` · the ratification record's remaining live rows folded inline and the file removed — one source of truth.
...
6. **Three selections flow down any mount tree, each by its own law:** `read / write / run` — **reach**, intersecting downward, judged by the engine under its *call context* — the engine's own contract, which `view/mount` conforms to — exactly as for processes; **`draw`** — which implementations and templates a subtree admits, ordered, intersecting downward (permission and preference in one list); **`offer`** — elements a subtree makes available to descendants, ordered, accumulating downward with nearer shadowing farther, from which unbound optional `accepts` entries bind by the match. Each is a body key on any mount, readable at every node, a body edit at any closure. Reach and offer hold for process trees too; `draw` is mount-only.
...
Each `component/*` package ships its component declarations, their payload archetypes, and their default implementations (as a program ships body and executable); a second package may implement the same declarations differently. **The dependency law [R, re-worded for surface kinds]: declarations depend on declarations; implementations depend on declarations plus a surface kind; nothing ever depends on an implementation.** **A module is a store** — each line above is a store directory by the settled recognition (`.ol/` inside, db and toml within), the module's files beside it; dependency is attach (*engine*). Migration: `host/react` → `component/base` · `host/programs/*` → `component/*`, `desktop/`, or retired · `host/` → `chassis-desktop` + `runtime-vm` · `engine/sdk` stays, `view/sdk` is new.
...
Carried unchanged. **A store** is a directory carrying `.ol/` — db and toml within; the `.ol/` is the recognition **[R — the settled convention; the feedback fold re-affirmed it]**. **Attach** brings a store into the running field. **One connection, all attached stores, one evaluator [P — supersedes settled engine law]** (per-store-evaluate-then-union provably leaks). **Generated ids become blocking [R].** **Commits are per-store [R]**; writes **route** to the owning store [P]. Opens: the id-debt schedule.
...
- **The engine serves sources [R — the feedback fold].** `ol://` resolves through the engine — *file of store s at path p, at the attached commit* — so no client knows disk paths and the web flavor is unchanged. Constraint it imposes: a module's files live under the store's version control, so pinning a commit pins chunks and code together. Files may become their own integration down the line; the pilot serves through the engine.
- **Programs are headless [P]**: `runtime: vm | native`; `webview` retires.
- **The call context [R — the direction: the engine never names view; the shape and the name are steward-drafted, P].** The author's catch: `Context { mount }` put a view archetype in the engine. The ruled direction: the engine defines its own **structural contract** and **`view/mount` conforms from the view side** — the engine never references a view archetype (the dependency law holds at this seam too: view depends on engine, never the reverse). The steward-drafted shape [P]: a conforming chunk names a *declaration* carrying ceilings, an *argument* (a selection), its own *grant* (`read`/`write`/`run` additions), and has a derivable *parent* conforming chunk; effective reach at a link = (the argument, read-granted as any argument is ∪ the declaration's ceiling ∪ the grant) ∩ the parent's; the chain caps at the **machine context** — `Context::process_id = None`: full reach over what is attached, every created chunk naming its owner. A process is the near-conformer, not a conformer by construction: its **frame** is a fifth reach source, exempt from the parent cap — where the frame sits in the generic contract is the rewrite's to draw [O]. `Context` gains **`{ anchor }`** [P — the name] — a ref to any conforming chunk; batch reads carry the anchor per entry; the engine derives the chain from field data — cacheable — and judges `read` / `commit` / starts as for a process; rendering under a mount reads under its anchor's reach. **Intents are dissolved [R]**: what interface code emits is an ordinary `commit` or start. How the parent link is derived (containment backrefs vs a stored key) [O — encoding].
- **Starts from a mount are `launch` [P].** ("mount" here and below is view's — *view/mount* — never the attached store.) A mount has no frame; a process it starts is owned by the **configured owner** — session is desktop-module state, not an engine concept [R — 2026-08-20]; the pilot desktop configures its session as that owner — capped by the mount's reach (the `run` wall says *which* programs; `launch` is the mode), watched by subscription. Other ownership mechanics when users and permissions arrive — out of scope.
- **Daemons [R — 2026-08-20].** A component that needs a running service names an ordinary program — impure programs are as launchable as pure ones; the walls (`run`, capabilities) govern, and purity is never the gate (it only decides what the planner may evaluate as an expression). **Nothing auto-starts**: in the pilot a daemon is started by a person's act — `launch` from the interface, or outside the field entirely — and a component whose service is absent draws the honest fault face; the docker-cli-without-engine posture, accepted for now. Whether a component may ever dispatch a start without a gesture is deferred with it [O].
...
Opens: an act journal as the escape if a truly atomic cross-store case appears (never spanning commits) · dynamic-attach encoding · ops beyond read/write/run under `{ anchor }` (`cancel`, `subscribe`) · the call chain's parent derivation · the frame's seat in the call contract (cap-exempt, processes only) · the consent chord's principal (lean: the mount of the draft face — *component/process*).
...
- **reader** — `[ ref(reading) ]`; places `reading.current.mounts` via `ctx.mount`; a drop → a new collation (its own act); its defaults table is the ruled reader-owned preferences. Opens: members shown together, as tabs when they cannot fit, or as a limited foldable overview [O] · attributes — per-element adjunct seats, dropped from an earlier generation; carry or not [O] · the marking/chrome taxonomy, made perfectly clear [O] · differencing [O] · the interaction drafting unit — click a chunk → a selection in the collation that another member takes as argument, with worked mixed-selection examples [O].
- **table** — `chunk-table`: the universal chunk component and last resort; field editors by type, reach facts deciding editor or text; over a read-only store editors render as text (no write). Kv over the body only, or the full chunk face with an embedded body table [O].
...
- **command** — entries derived from records + field (one per payload archetype, declared `actions` at two scopes); palette and entity menu, seated into overlay; the pick executes under the summoning mount's reach. Scope beyond commands [O — processes and recents as quick actions, the palette → reader → process chain as a task manager, shortcuts and their landing modes, whether the palette defaults to overlay]; mount history is a temporal read over mounts (they are chunks), locks are `view/locked` — a browse over them is the desktop's to add.
...
**What you run is the engine, and a chassis.** Two commands, one directory — **and the home is a store directory like any other [R — the feedback fold; `field.ol` was the deviation]**, recognized as every store is: `.ol/` inside, db and `project.toml` within (the established naming kept [R — 2026-08-20]; the *contents* are superseded — `[[attach]]` replaces `[[mounts]]`, `[chassis]` is new):

```
~/.config/ol/              the home — a standard store directory; the personal store by default, not by kind
  .ol/
    db                     the personal store: sessions, dynamically attached stores, your own edits
    project.toml           [[attach]] entries (path, branch, at?, write) · [chassis] entry = <chunk>
```

Because the home is nothing special, **`--home` may point at any store directory — a project included**: the field you boot into is that store, and a session sits in the store it was opened for — the project-agent line (*agent*) cashed with zero mechanism. `ol engine --home …` (binary or OS service — home is its only argument) opens the store, attaches every `[[attach]]` entry, then what the field records as dynamically attached. `ol desktop --home …` connects, reads the toml's `entry` (or `--entry` / `--mount`), and hosts the surface. **The toml is the declared, version-pinned set — your package space, editable in any editor**; dynamic attachments (opening a project) are recorded in the field, not the toml: *declared* and *opened* are two different things with two homes, no duplication. First run seeds the home from the engine distribution's bundled stores (`component/base`, `desktop/`, the guide) — packaging, not dependency; flags override config; **the toml never grows a second home**.
...
- **the shell — the simple tiler [tabs and sidebar out of the pilot shell: R; the verb set steward-drafted: P].** `desktop/shell : view/component { accepts: [ ref(session) ] }` realized **as a template** placing `$session.root` — a tree of `split` and content mounts; the arrangement chunks shrink to `session { root?: ref(view/mount) }` (`tab` dies with the tab strip; `host/tile` and `host/overlay` were already gone). The tiler is verbs and one gesture, no mechanism of its own: **split** — a command verb from the match, reached by the secondary gesture (who declares it is unpicked — `actions` sit on programs/components at two scopes; split/close's declarer is the rewrite's [O]), wrapping the target mount in a new `split { direction, [target, new] }`, the new pane filled by the defaults table's last resort until content is chosen; **close** — the ruled body edit, removing the mount from its parent's list, a single-child `split` collapsing into its child (the verb's own normalization); **resize** — the divider is the split implementation's own pointer interaction (the ordinary gesture belongs to the component in use), committing `ratio`; **move/swap** — post-pilot, riding the drag layer, and WYSIWYG rearrangement with it; the tiler needs nothing from that layer to be usable. With tabs gone the shell is nearly transparent — its remaining duty is resolving *which* session (the session, below);
- **`sidebar`** — out of the pilot shell; remains a declared component in the module (the session rendered as itself, work only — processes mean work; whether mounts show too [O]), mountable like any content;
- **the session — minor state of the pilot desktop module [R — 2026-08-20].** Not an engine concept (the engine's `launch` owner is configured — *engine*). Two duties earn it: the **writable indirection** — the entry is shipped read-only, so the live arrangement needs a writable home, and `$session.root` is that seam — and the **work group** — launched processes owned by the session, so kill cascades and the sidebar has a "work" to render. `session { root }` sits in the store it was opened for;
...
Engine as artefact + attach-era db (attach record, `attach`/`detach`, `[engine/attached]`, engine-served sources) · chassis-desktop hosting `web-dom` (entry, layers, reservations, the shorthand) · `engine/sdk`, `view/sdk` with `solid()` · `component/base` (leaves, `list`, `split`, faces, `FrameBox`) · `desktop/` (entry, the shell template — the simple tiler, projects) · reader · table · process · command · overlay · secrets · agents. After: `runtime-vm`, prose, drag/WYSIWYG (move/swap rides it), tabs and sidebar in the shell, lift, `GLBox`, code-creating mounts, generated types.

## Open — the index

db: id-debt schedule · engine: act journal, dynamic-attach encoding, ops beyond read/write/run under `{ anchor }`, the call chain's parent derivation, the frame's seat in the call contract (cap-exempt, processes only), the consent chord's principal · view: the ref refinement for closed slots (new type grammar), expressions-in-fields encoding (slots, templates, offers), nearest-vs-root ordering for `draw`, the duplicate-realization governance rule's exact statement, when code-creating mounts arrive · web-dom: record and connection encodings, ephemera home, web input floor, the slot-drop interface · base: grid or lists, the spacing/highlight law · desktop: the tiler verbs' declarer, the sidebar's mounts question, the projects component's shape · reader: members-together/tabs/overview, attributes, the marking taxonomy, differencing, the interaction drafting unit · table: kv or the full chunk face · command: scope beyond commands, a mount-history browse · engine also: gestureless starts (deferred with the daemon posture) · chassis and home: flavors, reservation encodings, the attach-time consent chip, the seed's contents · **to prototype before folding: `ctx`, the adapters and the typed-argument ergonomics — the authoring experience.**
