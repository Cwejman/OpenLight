...
> **Superseded twice, author-ruled.** 2026-08-07: `ref` admitted (§3 note below). 2026-08-12: **`selection = list<loc | ref | expr>` — ordered, duplicates rejected as a stated clause**; order is the consumer's to read (the model family reads window order; walls ignore it). "Arguments are sets" (§4) survives in substance — no names, no assignment step, structural matching — but the value carries order. See conclusions.md §E addendum.

A selection is **what you offer to be viewed or consumed**: places, and derivations of places. ~~To offer one chunk, offer the place at it — `[c]`, one term (which carries both the chunk and its members — §2). A bare `ref` never appears in a selection: refs are pointers living in bodies.~~ **(Superseded 2026-08-07, author-ruled: `selection = set<loc | ref | expr>`.** `[c]` is *not* the single-chunk case — §2 defines it as the chunk **and what is placed on it**, so X ≠ [X] and offering a bare chunk had no expressible form short of subtracting a place's members from itself. §4's own matching rule already presupposed ref elements, or its `ref(X)` entries could bind nothing. *Refs are pointers living in bodies* survives as a claim about storage, not about what may be offered.**)** An `expr` element takes two forms: **inline** (the anonymous graph — prose fences) or **a reference to an expression chunk**; in arguments and collations it is always the chunk form (§4, §12).
...
- ~~`ref` as a selection element (one-term locations cover the single-chunk case).~~ **Reversed 2026-08-07 (author): the stated reason was false on this file's own §2 — a one-term location carries the chunk *and* its members, so it never covered the single-chunk case. `selection = set<loc | ref | expr>`; see §3.**
