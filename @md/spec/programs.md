...
chunk host/overlay { instance: { anchor: ref, content: ref(engine/process), grade?: string } }
...
- **`overlay`** — a surface rendered above the composition, all fields, **no placements**: owned by the session (ownership is membership — it is already a `[session]` member), `anchor` names the span target, `content` the process rendered, `grade` the chosen form. Transient: content exits → the placer removes it; overlay-ness confers no reach.
...
**Overlays** (ruled 2026-08-12; learnings expected at build). Rendering is the **base page's** — host machinery ([`host.md`](host.md)): it watches `[session]`, mounts `content` when an overlay arrives, positions by anchor kind (point-anchored → at the anchor seat's measured rect; session-anchored → centered), and realizes the **dispatch-chosen grade's** bounds clamped to the viewport — for overlays no prior box exists, so the summoner names the grade and the box derives from it; expand is a re-summon at the larger grade. Creating an overlay needs **write over the anchor chunk** — nothing inherited; summoning is therefore an **intent to the anchor-holder** (the root program for tiles and the session; the host for the leader key), whose recorded write it becomes. Any surface program may be overlay-run at a grade — the command program is the first user, not a special case. Follow-up surfaces **inherit the summoning overlay's anchor**, never the click point.

**Writers.** The tile tree has exactly two: the **`arranger`** (§7 — renamed from `board`, author-ruled), which takes intents from any program, and the **shell**, which commits its own direct manipulation. With tiles owned flat under the session, both state the one-hop boundary `[session]` — tree edits are body edits on session, tab and tile chunks, all one hop from the root they hold write over.
...
Draws the session's processes from the field — `[session, engine/process]`, joined with what a usable rendering needs (program names, statuses, tile references) — plus whatever else the session explicitly holds, which is why it never lies. What it lists is what its boundary admits, like every other answer (substrate.md, *Boundaries*); there is no privileged view of a full set. `accepts: [ ref(session) ]` — the session itself, bare; reads `{[session], [engine/program]}` (processes are session members — one hop), writes `{[session]}`.

**The chrome suite is pinned** (ruled 2026-08-12). The shell's sidebar and tab-bar regions are seats with **pinned** surface calls — `sidebar([session])`, `tab-bar([session])` — deliberate seats, no match, no swap; both are ordinary programs (`accepts: [ ref(session) ]`; tab-bar reads/writes `{[session]}`), started by the slot layer as the shell's children — chrome lives with the window, correctly, since all view state is in the field. *Direction, later:* the root as a pure tiler with chrome as choices — a more modular environment; not the pilot's.
...
The palette is **the command program at its session-anchored grade** (§5 — the point-anchored menu is the same program's other form; one name owed). Leader key opens it — started by the host, which places the overlay, since the program holds no write reach over the session. One input; four modes, switched by prefix or arrow:
...
5. **Go** — a split button, authored by the composing surface (process-view's is the vanilla one). **Starting and landing are two acts**: the main press consumes the draft (`launch`) and fires the default landing — an `arranger/open { content, position }` intent; the caret opens an ordinary value select over `read([arranger/position])` (`here` · `beside` · `tab` · `overlay(grade)` · `none` — value chunks, so a new landing kind is a committed chunk, not UI code). Defaults by the program's nature: surface → *beside*, VM → *none*. The same calls appear as entries in the draft's context menu; the button is those calls pinned visible. A program whose `accepts` has no required entries and whose ceiling is already exact runs on plain Enter, with nothing to compose.
...
**Verbs from the field** (ruled 2026-08-12; UX grounded in [`research/arc/menu-scenes.md`](research/arc/menu-scenes.md); learnings expected at build). The mechanism, whole:

- **The gesture is the seat's.** Secondary click is caught by the slot machinery — never the surface — which packages the offer (the seat's selection) and the anchor, and sends the intent to the anchor-holder. Ordinary clicks stay the surface's events. A click on nothing resolves to the nearest enclosing seat.
- **The entry grain is the payload archetype**, not the program: one arranger, many verbs (`arranger/split`, `arranger/close`…), one entry per payload the offer can fill, labeled by the archetype. Multi-select is free — the offer is a selection and the match counts (`compare` appears for exactly two commits).
- **Assembly is field reads only** — nothing running is asked anything: the registry match over `[engine/program]` against the offer (the bulk, automatic); **declared `actions`** — an optional body key naming programs or single payload archetypes — read from two scopes: *the render chain* (view verbs: a reader declares hide/include/read-in-place; absent where no reader embeds) and *the offer's own types* (thing verbs: `agent` declares its steering payloads, so they appear wherever a turn is clicked); derived seat choices (surface, grade — §1, §3); built-ins. A declared action still only appears if the offer fills its payload.
- **One command program, two grades** — the point-anchored menu form and the session-anchored palette form are the same program at dispatch-chosen grades (its name is open); search in both, shortcuts as bound entries with their chrome on items, expand = re-summon at the larger grade. The pick executes under the menu's grant — excess drafts (run-to-draft) — then the program exits and the placer removes the overlay. The offer, the entry list (a payload chunk), the pick and the act are all recorded.
- **Context menus versus value selects, the line**: entity menus (field-assembled verbs) are infrastructure and may never be surface-drawn; value selects — input to an act a surface is composing — are always the surface's own UI (§2, the run button). Menus built inside individual programs are interim only.

*Open, expected to move at build:* state-dependent enablement (entries are static in v0.1); the command program's name; the verb-program homes (`curate`, `steer` as working names).
...
| `sidebar`, `tab-bar`, `palette` | webview | chrome as programs — sidebar and tab-bar in the shell's pinned seats, palette as the host-placed session overlay (§1) |
| `arranger`, `group` | vm | arrangement writer (renamed from `board`); container lifecycle (§7) |
