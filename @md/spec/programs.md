...
```ol
chunk host/session { instance: { name?: string, current-tab?: ref } }
...
- **`session`** — the outer container, restorable and shareable. Tabs and processes are placed `instance` on a session instance; any process on the session is sidebar-visible, so session membership *is* sidebar presence — no pin archetype.
- **`tab`** — the root of a tile tree, placed on its session. Workspaces are tabs: one term, one archetype.
- **`tile`** — a node in the split tree, placed on its tab or its parent tile with `seq` choosing the split side. That `seq` is why both `host/tab` and `host/tile` declare `seq: true` — an ordered place is an instance of a `seq: true` archetype (substrate.md, *Ordered places*). A split node carries `direction` (`horizontal` divides the width, `vertical` the height) and `ratio` (the seq-first child's share, clamped inside 0–1); a leaf carries neither, and renders whatever process is `relates` on it.
...
- **`recipe`** — a tile subtree preserved for spawning: tiles placed on the recipe, cloned under a chosen root at spawn. The recipe persists separately from anything spawned from it; the live form is a **composition** — a container process visible as one unit in the sidebar with nested tiles on the board. Collapsing the container stops its children.

*Open — the containment kind.* The tree uses `instance` for containment (tab-on-session, tile-on-tile), which reads oddly now that each connection kind carries exactly one meaning and `instance` means *what it is*. `owned` is the kind that means *where it lives*, and it would make the tree walkable as ownership — but it also fires substrate.md's naming rule (*a chunk that has members must have a name*) on nameless tiles, the same rule already open for process chunks. Marked, not decided; changing it also moves [`bootstrap.md`](bootstrap.md)'s seed.
...
**Writers.** The tile tree has exactly two: the **arranger** (`board`, §7), which takes intents from any program, and the **shell**, which commits its own direct manipulation. The shell's boundary must therefore state depth — a bare `[session]` admits one hop, and tiles sit two hops down (on tabs), so the shell and tab-bar write `[session] | follow` (a `follow`-shaped term, [`engine.md`](engine.md), *Boundaries*; the walk's parameterization is the planner's). This is the correction the one-hop reach law forces: a root grants membership, not a subtree.
...
2. **Boundary chips.** The run's `read` and `write`, as the engine will construct them from the five sources: the frame, argument content (read-granted implicitly by the offer), the program's stated ceiling, explicit additions, and the parent cap ([`engine.md`](engine.md), *Boundaries*). A stated ceiling shows as the ceiling — a run narrows, never widens. `read: {}` / `write: {}` shows as *frame only*: `model`, `web` and `filesystem` reach nothing beyond their own frame, enforced rather than promised.
...
Three faces are drawn explicitly, never silently: a **dead root** (an empty place invites; a dead one does not), a **reference error** (inline, quietly), and a **beyond-reach reference** (§1 — nothing runnable resolves there because the boundary does not admit it, and the surface says so). Unnamed chunks show truncated ids. Only `document` distinguishes membership kinds visually (*open for revisit*; `document`'s own dissolution into `prose` + placement chrome is flagged, awaiting the author's word). `db/commits` needs no special surface — an ordered place served by `sequence`, each commit showing its message and its touched addresses as chrome, history dissolved. *Open ([`engine.md`](engine.md), What Is Open):* `db/commits` is projection-backed, and a projected archetype cannot carry a stored `seq: true`.
