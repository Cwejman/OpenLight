...
2. **The knowledge arc** — [`code.md`](spec/research/knowledge/code.md) stands **ratified** (2026-08-30); the search's working state — focus, claims under test, evidence, method — lives in [`study.md`](spec/research/knowledge/study.md), and the arc's served records (opening, the two derivations, the tree survey) are retired to git. Next in the arc, in order: **labs on the sweep corpus** — the claims tried on easy ground first (study.md holds the claims and the tests) — then **the first proof** — the markdown-directory piece the code's empty link points at, deliberately unwritten until it can be harvested from what the labs settle — then **the spec/ rewrite under code + proof** (this absorbs the register pass; the arc's finish line). Still awaiting the author's read: the sweep's three artifacts ([`findings/`](spec/research/sweep-2026-08/findings/README.md), [`encounters.md`](spec/research/sweep-2026-08/encounters.md), [`method.md`](spec/research/sweep-2026-08/method.md)); [`landscape.md`](spec/research/landscape.md) kept as ore. Standing beside: conventions.md's remaining turnover (its knowledge section is superseded and points at the code; the rest still owed the README treatment) · this board (keep it pure state).
