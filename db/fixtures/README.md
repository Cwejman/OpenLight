...
Ground truth for the consumer ↔ db contract, written from [`pilot/substrate.md`](../../pilot/substrate.md) before any implementation. Every case is a claim the spec makes; the db crate's tests and the SDK's tests both consume these files through thin adapters, so the two implementations cannot drift apart silently.
